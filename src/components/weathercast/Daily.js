      ...
      {display === true &&
        <Fragment>
          ...
          <h2>Daily</h2>
            <div className='scroll-div'>
              {weatherData.daily?.slice(1).map(daily => (
                <div
                  className={chosenDay === daily.dt ? 'scroll-box-clicked' : 'scroll-box'}
                  id='scrollBoxId'
                  key={daily.dt}
          ...
          <div className={`daily-details ${isVisible}`}>
            ...
            <div>
              <h4>{currentHour(dailyDetailsWeather.sunrise, weatherData.timezone_offset)}:{currentMinutes(dailyDetailsWeather.sunrise)}</h4>
              <p>Sunrise</p>
              <h4>{currentHour(dailyDetailsWeather.sunset, weatherData.timezone_offset)}:{currentMinutes(dailyDetailsWeather.sunset)}</h4>
              <span>Sunset</span>
            </div>
          </div>
        </Fragment>
